...
void Interface<T>::menu()
{
    ...
    do
    {
        ...
        case 3:
            ...
            else  std::cout << "Стек пуст, нечего удалять." << std::endl;
            break;
        }
        case 4: {
            if (!st.empty()) modifyElement(st);
            else  std::cout << "Стек пуст, нечего изменять." << std::endl;
            break;
        }
        case 5: {
           // if (!st.empty()) searchVehicle(st);
           // else std::cout << "Стек пуст, поиск невозможен." << std::endl;
            break;
        }
        case 6: {
...
void Interface<T>::func()
{
    ...
    {
        system("cls");
        std::cout << "1. Работать с грузовым ТС" << std::endl
        << "2. Работать с пассажирским ТС" << std::endl
        << "3. Работать с легковым ТС" << std::endl
        << "4. Выход." << std::endl
        << "Ваш выбор: ";
        std::cin >> choice;
        switch (choice)
...
void Interface<T>::modifyElement(stack<T*>& st) {
    ...
    if (st.empty()) std::cout << "Элемент с таким номером отсутствует." << std::endl;
    else {
        elementToModify = st.pop1();
        std::cout << "Текущие данные элемента:" << std::endl;
        std::cout << *elementToModify << std::endl;
        std::cout << "Выберите поле для изменения:" << std::endl;
        std::cout << "1. Цвет" << std::endl;
        std::cout << "2. Бренд" << std::endl;
        std::cout << "3. Модель" << std::endl;
        std::cout << "4. Год выпуска" << std::endl;
        std::cout << "5. Лошадиные силы" << std::endl;
        std::cout << "6. Тип двигателя" << std::endl;
        if (dynamic_cast<Car*>(elementToModify)) {
            std::cout << "7. Вес автомобиля" << std::endl;
            std::cout << "8. Количество дверей" << std::endl;
        }
        if (dynamic_cast<SportsCar*>(elementToModify)) {
            std::cout << "9. Максимальная скорость" << std::endl;
            std::cout << "10. Время разгона до 100 км/ч" << std::endl;
        }
